...
assert.match(renderer, /<label class="transaction-card-check" aria-label="Selecionar[^`]+<input data-select-transaction="\$\{transaction\.id\}" type="checkbox"\$\{selected \? " checked" : ""\} \/>/, "Transaction cards must expose a single accessible checkbox inside their circular label");
assert.match(activeSkin, /\.app-shell \.transaction-card-check\s*\{[^}]*display:\s*flex;[^}]*align-items:\s*center;[^}]*justify-content:\s*center;[^}]*width:\s*var\(--size-touch-target\);[^}]*min-width:\s*var\(--size-touch-target\);[^}]*height:\s*var\(--size-touch-target\);[^}]*min-height:\s*var\(--size-touch-target\);[^}]*cursor:\s*pointer;/, "The full transaction selection circle must center its native checkbox and retain the shared touch target");
assert.match(activeSkin, /\.app-shell \.transaction-card-check input\s*\{[^}]*width:\s*var\(--size-icon-small\);[^}]*min-width:\s*var\(--size-icon-small\);[^}]*height:\s*var\(--size-icon-small\);[^}]*min-height:\s*var\(--size-icon-small\);[^}]*margin:\s*0;[^}]*padding:\s*0;/, "The card checkbox must override the global form-control minimum height without fragile offsets");
assert.match(activeSkin, /\.app-shell \.transaction-card-check:focus-within\s*\{[^}]*box-shadow:\s*var\(--nx-focus-ring\);/, "Keyboard focus must remain visible around the complete transaction selection target");
assert.doesNotMatch(activeSkin.match(/\.app-shell \.transaction-card-check input\s*\{[^}]*\}/)?.[0] || "", /margin-(?:top|right|bottom|left):\s*-/i, "Checkbox alignment must not depend on negative margins");
assert.match(index, /data-select-all-transactions type="checkbox" aria-label="Selecionar transa[^\"]+"/, "Visible transaction selection must retain its accessible bulk checkbox");
assert.match(renderer, /\[data-select-transaction\][\s\S]*?checkbox\.addEventListener\("change"[\s\S]*?selectedTransactionIds\.add[\s\S]*?selectedTransactionIds\.delete/, "Individual checked and unchecked states must keep using the existing selection set");
assert.match(renderer, /\[data-select-all-transactions\][\s\S]*?rows\.forEach\(\(transaction\) => state\.selectedTransactionIds\.add[\s\S]*?rows\.forEach\(\(transaction\) => state\.selectedTransactionIds\.delete/, "Selecting visible transactions must keep supporting multiple selection and clearing");
assert.match(renderer, /selectAll\.indeterminate = Boolean\(visibleSelected\.length && visibleSelected\.length < visibleIds\.length\)/, "Partial multi-selection must retain the indeterminate state");
assert.match(legacyComponents, /@media \(max-width: 767px\)[\s\S]*?\.transaction-card-grid,[\s\S]*?\.transaction-card,[\s\S]*?max-width:\s*100%;[\s\S]*?\.transaction-card-grid\s*\{[^}]*grid-template-columns:\s*1fr;/, "Mobile and Fold transaction cards must remain in one overflow-safe column");
assert.match(components, /body\.theme-dark[\s\S]*?\.transaction-card-check[\s\S]*?border-color:\s*var\(--input-border\);[\s\S]*?background-color:\s*var\(--input-background\);/, "The transaction checkbox circle must retain dark-theme contrast");
assert.match(activeSkin, /\.transaction-card-check:focus-within/, "The shared Web asset must carry the same focus and alignment contract into Android sync");
assert.match(index, /data-sync-card data-sync-tone="neutral"[\s\S]*?role="status" aria-live="polite"[\s\S]*?data-sync-description[\s\S]*?data-sync-action/, "Synchronization status must expose readable text, an announced description, and an optional recovery action");
assert.match(renderer, /currentSyncPresentation\(\)[\s\S]*?presentation\.description[\s\S]*?card\.dataset\.syncTone[\s\S]*?presentation\.actionLabel/, "Synchronization presentation must update its explanation, semantic tone, and recovery action together");
assert.match(activeSkin, /settings-sync-card\[data-sync-tone="success"\][\s\S]*?var\(--nx-income\)[\s\S]*?settings-sync-card\[data-sync-tone="warning"\][\s\S]*?var\(--nx-warning\)[\s\S]*?settings-sync-card\[data-sync-tone="danger"\][\s\S]*?var\(--nx-expense\)/, "Synchronization tones must use shared theme tokens while retaining textual status");
assert.match(activeSkin, /@media \(max-width: 900px\)[\s\S]*?settings-sync-card\s*\{[\s\S]*?grid-template-columns:\s*minmax\(0, 1fr\);/, "Synchronization status must collapse to an overflow-safe mobile and Fold layout");
